...
- (void)updateIndicators {
	...
	BOOL isActivityIndicator = [indicator isKindOfClass:[WLIActivityIndicator class]];
	BOOL isRoundIndicator = [indicator isKindOfClass:[MBRoundProgressView class]];
	
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		// Update to indeterminate indicator
		[indicator removeFromSuperview];
		self.indicator = MB_AUTORELEASE([[WLIActivityIndicator alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 37.0f, 37.0f)]);
		[(WLIActivityIndicator *)indicator startAnimating];
